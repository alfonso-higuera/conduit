...
export const ConduitLink = props => {
  let {to, name, ...linkProps} = props;
  if (linkProps.absolute) {
    return <Link to={to} {...linkProps}>{name}</Link>;
  } else {
    return <Link to={`${pathPrefix}${to}`} {...linkProps}>{name}</Link>;
  }
};
        ...
        <Layout.Content style={{ margin: '0 0', padding: 0, background: '#fff' }}>
          <div className="main-content">
            <Switch>
              ...
              <Route path={`${pathPrefix}/servicemesh`} render={() => <ServiceMesh api={api} pathPrefix={pathPrefix} releaseVersion={appData.releaseVersion} />} />
